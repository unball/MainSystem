...
from view.viewThreads import ViewLoopThread
import cv2
import time

class cv2Renderer(Gtk.Frame):
  ...
  def __init__(self, worker=None, interpolation=cv2.INTER_LINEAR, widthHeightProportion=471/350):
    ...
    if worker is not None:
      self.__thread = ViewLoopThread(self.get_worker_frame)
    else: self.__thread = None
  ...
  def start(self):
    ...
    if self.__thread is not None: self.__thread.start()
