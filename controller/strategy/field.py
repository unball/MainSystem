...
from controller.tools import ang, unit, angl, angError, norml
import numpy as np
...
class UVF():
  ...
  def F(self, P, retnparray=False):
    if len(P.shape) == 1: P = np.array([P]).T

    # Ajusta o sistema de coordenadas
    Pb = np.array([self.Pb])
    P[:2] = P[:2]-Pb.T[:2]
    rotMatrix = np.array([[np.cos(self.Pb[2]), np.sin(self.Pb[2])],[-np.sin(self.Pb[2]), np.cos(self.Pb[2])]])
